...
class CryptoService {
  ...
  Future<void> init() async {
    ...
    final idSkBytes = await idKp.extractPrivateKeyBytes();
    ...
    await _storage.write(
      ...
      value: base64Encode(idPub.bytes),
    );
    ...
    final spSkBytes = await spKp.extractPrivateKeyBytes();
    ...
    final signed = await _ed25519.sign(spPub.bytes, keyPair: signKp);
    ...
    for (var i = 0; i < 10; i++) {
      ...
      final kpSk = await k.extractPrivateKeyBytes();
      otpkList.add({'keyId': id, 'publicKey': base64Encode(kpPub.bytes)});
  ...
  Future<String> encryptFor(String recipientId, String plaintext) async {
    ...
    final rpIdentity = base64Decode(bundle['identityKey']); // X25519 pub
    final rpSignedPre = base64Decode(bundle['signedPreKeyPublic']);
    ...
    if (rpSigningPub != null && bundle['signedPreKeySignature'] != null) {
      try {
        final ok = await _ed25519.verify(
          ...
          publicKey: SimplePublicKey(rpSigningPub, type: KeyPairType.ed25519),
        );
    ...
    final myIdSk = base64Decode(myIdSkB64);
    final myIdPub = base64Decode(myIdPubB64);
    final mySignedSk = base64Decode(mySignedSkB64);
    ...
    final bytes = <int>[];
    bytes.addAll(await dh1.extractBytes());
    bytes.addAll(await dh2.extractBytes());
    bytes.addAll(await dh3.extractBytes());
    if (dh4 != null) bytes.addAll(await dh4.extractBytes());
  ...
  Future<String> decryptEnvelope(String envelopeJson) async {
    ...
    final ephPub = base64Decode(header['ephemeral'] as String);
    final senderIdentity = base64Decode(header['senderIdentity'] as String);
    ...
    if (usedOneTimeId != null && otpkMap[usedOneTimeId] != null) {
      final otPriv = base64Decode(otpkMap[usedOneTimeId]);
